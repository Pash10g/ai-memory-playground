    ...
    try:
        ...
        if message_input.type == "human" and len(message_input.text) > 30:
            try:
                ...
                logger.error(f"Failed to create memory for user {message_input.user_id}: {memory_error}")
                raise memory_error
        else:
            try:
                ## add memory for AI messages
                if message_input.type == "ai" and len(message_input.text) > 30:
                    memory_content = (
                        f"From conversation {message_input.conversation_id}: {message_input.text}"
                    )
                    logger.info(f"Creating memory for AI response for user {message_input.user_id}: {memory_content}")
                    await remember_content(
                        RememberRequest(user_id=message_input.user_id, content=memory_content)
                    )
            
            except Exception as memory_error:
                logger.error(f"Failed to create memory for AI response for user {message_input.user_id}: {memory_error}")
                raise memory_error
